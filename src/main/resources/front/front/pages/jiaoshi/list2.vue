	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.jiaoshiName" type="text" placeholder="教师名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('jiaoshi','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.jiaoshiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('jiaoshi','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.jiaoshiPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">教师手机号</text>
                     <text style="width: 70%">{{item.jiaoshiPhone}}</text>
                 </view>
                 <view v-show="item.jiaoshiIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">教师身份证号</text>
                     <text style="width: 70%">{{item.jiaoshiIdNumber}}</text>
                 </view>
                 ...
                 <view v-show="item.jiaoshiEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">邮箱</text>
                     <text style="width: 70%">{{item.jiaoshiEmail}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('jiaoshi','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                ...
                <!--<view v-if="isAuth('jiaoshi','审核') && item.jiaoshiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`jiaoshi`, {
                                id:id,
                            });
                            //await _this.$api.requestConditionDataGet('jiaoshi','zhifu',null,{
                            //    id:id
                            //});
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.jiaoshiYesnoTypes=shenheTypes;
                            data.jiaoshiShenheTime=shijian;
                            await _this.$api.update(`jiaoshi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
