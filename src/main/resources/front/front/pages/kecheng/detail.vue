	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
				...
				<view class="yinyuewai">
					<view class="yinyuenei">
						...
						<view class="shangpinfei">
							{{detail.kechengName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								:src="baseUrl+detail.kechengVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
				...
				<view @tap="fanhui" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		async onShow(options) {
			...
			await this.$api.save(`xuexizuji`, data);
		...
		methods: {
			...
			async submitData() {
				...
				let data = {
					...
					kechengLiuyanPingfenNumber: this.scoreData,
					kechengLiuyanText: this.commentData,
				}
				await this.$api.save(`kechengLiuyan`, data);
				this.$refs.deliver.close()
				this.mescroll.num = 1
				this.upCallback(this.mescroll)
			},
			...
			async fanhui() {
				...
				await this.$api.requestCondition("xuexizuji", "jieshuxuexi", data)
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('kecheng', this.kechengId);
				this.detail = res.data
				this.detail.kechengContent = this.detail.kechengContent.replace("<img", "<img  class=\"daxiao\"")
				...
				if (res.data.list.length == 1) {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('kechengCollection', JSON.stringify([
									kechengCollectionId
								]));
			...
			async getThumbsup() {
				...
				let res = await _this.$api.list(`kechengCollection`, params);
				if (res.data.list.length > 0) {
					if (res.data.list[0].kechengCollectionTypes == '2') {
						_this.thumbsupFlag = true;
					}
					if (res.data.list[0].kechengCollectionTypes == '3') {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('kecheng', _this.detail);
								_this.$utils.msg('取消成功');
			...
			async baoming() {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('kechengYuyue', {
								yonghuId: _this.user.id,
								kechengId: _this.detail.id,
							});
							_this.$utils.msg('报名成功');
							_this.$utils.jump('/pages/kechengYuyue/list');
						}
					}
				});
			},
