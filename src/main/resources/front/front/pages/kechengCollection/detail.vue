	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kechengCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kechengCollection', this.kechengCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.kechengCollectionPhoto ? this.detail.kechengCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kechengCollectionYuyue', {
									yonghuId: _this.user.id,
									kechengCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kechengCollectionYuyue/list');
							}
						}
					});
				},
