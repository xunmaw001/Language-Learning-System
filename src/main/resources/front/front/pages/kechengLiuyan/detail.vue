	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kechengLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kechengLiuyan', this.kechengLiuyanId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kechengLiuyanYuyue', {
									yonghuId: _this.user.id,
									kechengLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kechengLiuyanYuyue/list');
							}
						}
					});
				},
