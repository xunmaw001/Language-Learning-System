	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.kechengLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('kechengLiuyan','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
                ...
                <view style="font-size: 30rpx;">
                    ...
                    <b>{{item.kechengName}}</b>
                        ...
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.kechengLiuyanText}}</text>
                        </view>
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`kechengLiuyan`, {
                                id:id,
                            });
                            //await _this.$api.requestConditionDataGet('kechengLiuyan','zhifu',null,{
                            //    id:id
                            //});
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.kechengLiuyanYesnoTypes=shenheTypes;
                            data.kechengLiuyanShenheTime=shijian;
                            await _this.$api.update(`kechengLiuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
