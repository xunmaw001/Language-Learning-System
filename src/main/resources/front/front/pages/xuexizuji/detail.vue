	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xuexizuji', this.xuexizujiId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xuexizujiPhoto ? this.detail.xuexizujiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexizujiYuyue', {
									yonghuId: _this.user.id,
									xuexizujiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xuexizujiYuyue/list');
							}
						}
					});
				},
