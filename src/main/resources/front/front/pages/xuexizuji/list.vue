                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xuexizujiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xuexizujiTypesList" :key="index" :class="xuexizujiTypes===item.codeIndex?'tabActive':''"
                                  @tap="xuexizujiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.xuexizujiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('xuexizuji','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <view v-show="item.kechengName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">课程名称</text>
                    <text style="width: 70%">{{item.kechengName}}</text>
                </view>
                <view v-show="item.kechengTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">课程类型</text>
                    <text style="width: 70%">{{item.kechengValue}}</text>
                </view>
                <!-- 当前表 -->
                <view v-show="item.xuexizujiTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">开始时间</text>
                    <text style="width: 70%">
                        <text v-if="item.xuexizujiTime != null">{{item.xuexizujiTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                 <view v-show="item.xuexizujiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">学习状态</text>
                     <text style="width: 70%">{{item.xuexizujiValue}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('xuexizuji','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('xuexizuji','审核') && item.xuexizujiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 学习状态 字段 字典表查询方法
            async xuexizujiTypesSelect(){
                let xuexizujiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xuexizuji_types',
                });
                xuexizujiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xuexizujiTypesList = xuexizujiTypes.data.list
            },
            //当前表的 学习状态字段类别搜索
			xuexizujiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xuexizujiTypes != 0) {
                    params.xuexizujiTypes = this.xuexizujiTypes
                }
                let _this = this
                let res = await this.$api.page(`xuexizuji`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xuexizuji', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.xuexizujiYesnoTypes=shenheTypes;
                            data.xuexizujiShenheTime=shijian;
                            await _this.$api.update(`xuexizuji`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`xuexizuji`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
