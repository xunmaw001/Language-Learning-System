	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('xuexizuji','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
            ...
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                 ...
                 <view v-show="item.xuexizujiTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.xuexizujiTime}}</text>
                 </view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            this.categoryList = xuexizujiTypes.data.list
				...
				let params = {
					...
					limit: mescroll.size,
							...
							if (this.categoryName != 0) {
                                params.xuexizujiTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`xuexizuji`, {
                                id:id,
                            });
                            //await _this.$api.requestConditionDataGet('xuexizuji','zhifu',null,{
                            //    id:id
                            //});
